...
nextGuess (initialGuess, InitialGuess suitGuess) (corrects, lowerRanks, correctRanks, higherRanks, correctSuits)
  = let initMaxRank  = if higherRanks == 0 then R10 else maxBound
        ...
        miscGuess = [Card Club initMinRank, Card Club initMaxRank]
...
nextGuess (currentGuess, gs@GameReducing{}) (corrects, lowerRanks, correctRanks, higherRanks, correctSuits)
  = let nextMinRankGuess   = guessMinRank lowerRanks $ gsMinRankGuess gs
        nextMaxRankGuess   = guessMaxRank higherRanks $ gsMaxRankGuess gs
        clubIncluded       = elem Club . sgFound $ gsSuitsGuess gs
        correctSuitsOffset = if clubIncluded then -1 else 0
        nextSuitGuess =
            guessSuits (correctSuits + correctSuitsOffset > 0) $ gsSuitsGuess gs

        reduced =
            isRankGuessed nextMinRankGuess
              && isRankGuessed nextMaxRankGuess
              && isSuitGuessed nextSuitGuess
        nextGameState = if reduced
          then GameGuessing { gsMaxRank = getRank nextMaxRankGuess
                            , gsMinRank = getRank nextMinRankGuess
                            , gsSuits   = sgAnswer nextSuitGuess
                            }
          else gs { gsMinRankGuess = nextMinRankGuess
                  , gsMaxRankGuess = nextMaxRankGuess
                  , gsSuitsGuess   = nextSuitGuess
                  }

        baseGuess = makeNextReducingGuess (length currentGuess)
                                          (getRank nextMinRankGuess)
                                          (getRank nextMaxRankGuess)
                                          (getSuit nextSuitGuess)
    in  (baseGuess, nextGameState)
 where
  makeNextReducingGuess :: Int -> Rank -> Rank -> Suit -> [Card]
  makeNextReducingGuess count minRank maxRank suit =
    let baseGuess = [Card suit minRank, Card suit maxRank] in baseGuess
  --  makeNextAnswerGuess :: Int -> Rank -> Rank -> [Suit] -> [Card]
  --  makeNextGuess count minRank maxRank suits
  --   | not guessing
  --   =
...
testGame answer =
  ...
  in  loop initialGameState initialHistory 0 10
 where
  loop
    :: ([Card], GameState)
    -> [([Card], GameState)]
